...
The library reads/writes descriptive image metadata (EXIF, IPTC-IIM, XMP) for JPEG/TIFF/PNG. It is **under active construction** against the milestone plan. Done so far: scaffold, **JPEG read+write**, **PNG read+write**, **TIFF read**. Remaining: **TIFF write** (its own milestone — requires a pixel-preserving IFD rewriter, since Pillow re-encodes), directional **sync**, **XMP export** (sidecar/string) + custom namespaces, and required-field **validation**. Design decisions: mostly-pure-Python with permissive deps (`Pillow`, `piexif`, `defusedxml`) — no GPL/native `exiv2`/`libheif`; HEIC and camera-technical tags are deferred/out of scope. PNG has no standard IPTC slot; TIFF write is deferred, so `save()` raises `NotImplementedError` for TIFF.
